...
do
	...
	cmd="../line_vector -train ../test_cases/ref_inputs/reconstruct_$i.txt -output ../test_cases/r_outputs/line_$i.txt -binary $binary -size $dim -order $order -negative $num_negative -samples $samples -rho $rho -threads $threads"
	eval $cmd
done
