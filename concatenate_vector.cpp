...
static long long max_num_vertices = 1000, num_vertices = 0;
...
static void ReadVectors(std::vector<std::string> &first_order_vertices, std::vector<std::string> &second_order_vertices, 
	...
	/*rows = (long long) first_order_features.size(), cols = (long long) first_order_features[0].size();
	printf("%lld %lld\n", rows, cols);
	for (long long r = 0; r < rows; r++) {
		printf("%s ", first_order_vertices[r].c_str());
		for (long long c = 0; c < cols; c++) {
			printf("%lf ", first_order_features[r][c]);
		}
		printf("\n");
	}

	rows = (long long) second_order_features.size(), cols = (long long) second_order_features[0].size();
	printf("%lld %lld\n", rows, cols);
	for (long long r = 0; r < rows; r++) {
		printf("%s ", second_order_vertices[r].c_str());
		for (long long c = 0; c < cols; c++) {
			printf("%lf ", second_order_features[r][c]);
		}
		printf("\n");
	}*/
}
...
void ConcatenateMain(const std::vector<std::string> &first_order_vertices, const std::vector<std::string> &second_order_vertices, 
	...
	/*for (a = 0; a < num_vertices; a++) {
		printf("%s ", output_vertices[a].c_str());
		for (b = 0; b < vector_dim1 + vector_dim2; b++) {
			printf("%lf ", output_features[a][b]);
		}
		printf("\n");
		if (binary)
		{
