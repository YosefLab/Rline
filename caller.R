...
main <- function() {
  ...
  option_list <- list(
    ...
    make_options(c("-of", "--output_format"), type = "integer", default = 1, 
		help = "output file using (1) sprintf, (2) write.table, or (3) rds")
  ); 
  ...
  if (is.null(options$command)) {
    ...
    output(options$command, options$output_file, options$output_format, reconstruct_df)	
    ...
    output(options$command, options$output_file, options$output_format, line_matrix)	 
    ...
    output(options$command, options$output_file, options$output_format, concatenate_matrix)	 
    ...
    output(options$command, options$output_file, options$output_format, normalize_matrix)	 
  } else {
