...
graph <- function(name, input_matrix) {
	...
	plot <- ggplot() + aes(x  = input_matrix[, 1], y = input_matrix[, 2]) + geom_point()
        ggsave(filename = paste(output_dir, paste(name, "scatter_plot.pdf", sep = "_"), sep = ""), plot = plot, dpi = 100)
	plot <- ggplot() + aes(x = input_matrix[, 1], y = input_matrix[, 2]) + geom_density2d()
	ggsave(filename = paste(output_dir, paste(name, "probability_density_plot.pdf", sep = "_"), sep = ""), plot = plot, dpi = 600)
}

input_dir = "inputs/"
edge_list <- read.table(paste(input_dir, "10.in", sep = ""))
rline(edge_list, depth = 2, max_k = 10, neg = 5, samples = 10, rho = 0.025, name = "10")
